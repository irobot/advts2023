...
type Filter<LookFor extends string, ItemsToSearch extends Input, ItemsFound extends Input = []> = [
	...ItemsToSearch,
] extends [infer Head, ...infer Tail]
  // Ensure Tail is not empty
	? Tail extends Input
		? Head extends LookFor
		  // Item found, add it to the ItemsFound list
			? Filter<LookFor, Tail, [Head, ...ItemsFound]>
			// Skip current element, move on
			: Filter<LookFor, Tail, ItemsFound>
		// No more items left to search
		: ItemsFound
	: ItemsFound;

type Count<AllToys extends Input, ToyToCount extends string> = Filter<
	ToyToCount,
	AllToys
>["length"];
